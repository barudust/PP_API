...
async def registrar_venta(data: VentaCreateReq):
    async with database.transaction():
        ...
        for item in data.detalles:
            ...
            # Guardamos es_granel y la cantidad base descontada para poder
            # cancelar con exactitud (sin adivinar).
            q_detalle = venta_detalle.insert().values(
                ...
                cantidad_base=kilos_a_restar_total,
            )
            ...
            # Bloqueo de stock negativo (configurable). Al estar dentro de la
            # transacción, lanzar aquí revierte toda la venta (ACID).
            if nuevo_stock < 0 and not settings.PERMITIR_STOCK_NEGATIVO:
                raise HTTPException(
                    400,
                    f"Stock insuficiente de '{prod_db['nombre']}': "
                    f"disponible {stock_actual}, requerido {kilos_a_restar_total}.",
                )
            ...
            # F. Bitácora del movimiento de stock
            await registrar_movimiento(
                sucursal_id=data.sucursal_id,
                usuario_id=data.usuario_id,
                producto_id=item.producto_id,
                tipo_movimiento=MOV_VENTA,
                cantidad_anterior=stock_actual,
                cantidad_movida=-kilos_a_restar_total,
...
async def cancelar_venta(id: int):
    async with database.transaction():
        ...
        for item in items:
            # Preferimos la cantidad base guardada (exacta). Si es una venta
            # antigua sin ese dato, caemos al método de inferencia anterior.
            if item['cantidad_base'] is not None:
                kilos_a_regresar = float(item['cantidad_base'])
            else:
                q_prod = select(producto).where(producto.c.id == item['producto_id'])
            ...
            if inv:
                stock_previo = float(inv['cantidad'])
                nuevo_stock = stock_previo + kilos_a_regresar
                await database.execute(
                ...
                await registrar_movimiento(
                    ...
                    producto_id=item['producto_id'],
                    tipo_movimiento=MOV_CANCELACION,
                    cantidad_anterior=stock_previo,
                    cantidad_movida=kilos_a_regresar,
                    cantidad_nueva=nuevo_stock,
                    motivo=f"Cancelación venta #{id}",
                )
